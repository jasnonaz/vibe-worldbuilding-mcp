...
from ..config import TAXONOMY_OVERVIEW_SUFFIX, MARKDOWN_EXTENSION
...
def get_existing_taxonomies(world_path: Path) -> List[str]:
    ...
    if taxonomies_path.exists():
        for file_path in taxonomies_path.glob(f"*{TAXONOMY_OVERVIEW_SUFFIX}{MARKDOWN_EXTENSION}"):
...
def create_world_context_prompt(
    ...
    if existing_entries:
        ...
        for tax, entries in sorted(entries_by_taxonomy.items()):
            entry_names = [entry["name"] for entry in entries[:5]]  # Limit to 5 per taxonomy
    ...
    context_lines.append("\n**Linking Format:** Reference existing entries using their exact names in natural text for automatic crosslinking.")
...
def taxonomy_exists(world_path: Path, clean_taxonomy: str) -> bool:
    ...
    taxonomies_file = world_path / "taxonomies" / f"{clean_taxonomy}{TAXONOMY_OVERVIEW_SUFFIX}{MARKDOWN_EXTENSION}"
...
def create_basic_taxonomy(world_path: Path, taxonomy: str, clean_taxonomy: str) -> None:
    ...
    taxonomy_file = taxonomies_dir / f"{clean_taxonomy}{TAXONOMY_OVERVIEW_SUFFIX}{MARKDOWN_EXTENSION}"
...
- Role or significance in the world
- Relationships to other entries

### Optional Information  
- Detailed background and history
- Physical or conceptual characteristics
- Impact on the world or other entries

## Entry Structure

Entries in this taxonomy should follow a consistent format:
1. **Overview**: Brief introduction and key characteristics
2. **Details**: Expanded information and context
3. **Relationships**: Connections to other world elements
4. **Impact**: Significance within the broader world
"""
