...
from .utilities import (
    clean_name, get_existing_taxonomies, get_existing_entries_with_descriptions,
    taxonomy_exists, create_basic_taxonomy
)
...
def _create_stub_summary_response(created_stubs: List[Dict[str, str]], created_taxonomies: List[str]) -> list[types.TextContent]:
    ...
    if not created_stubs and not created_taxonomies:
        return [types.TextContent(type="text", text="No new stub entries or taxonomies were created. All suggested entities may already exist.")]
    ...
    if created_stubs:
        ...
        for stub in created_stubs:
            response_parts.append(f"- **{stub['name']}** ({stub['taxonomy']}): {stub['file']}")
