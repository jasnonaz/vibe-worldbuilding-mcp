...
class E2ETestRunner:
    ...
    async def setup_mcp_client(self) -> bool:
        ...
        try:
            ...
            self.log(f"MCP server initialized: {init_response.get('result', {}).get('serverInfo', {}).get('name', 'Unknown')}")
...
The Great Crystal Network is failing, causing islands to drift apart and magic to become unstable. Heroes must discover the cause and restore balance before the realm fractures completely."""
            ...
            taxonomies = [
                {"name": "characters", "description": "People, heroes, villains, and notable figures in the fantasy realm"},
                {"name": "locations", "description": "Magical places, cities, dungeons, and significant geographical features"},
                {"name": "artifacts", "description": "Magical items, weapons, and mystical objects of power"},
                {"name": "cultures", "description": "Societies, races, and civilizations in the fantasy realm"}
            ]
            
            response = await self.mcp_client.call_tool("instantiate_world", {
            ...
            if "successfully" in result:
                ...
                if world_dirs:
                    ...
                    missing_dirs = [d for d in required_dirs if not (self.world_directory / d).exists()]
                    
                    if missing_dirs:
                        self.record_result("World Instantiation", False, 
                                         f"Missing directories: {missing_dirs}", time.time() - start_time)
                        return False
                    
                    self.record_result("World Instantiation", True, 
                                     f"Created world with {len(taxonomies)} taxonomies", 
                                     time.time() - start_time)
                    return True
                else:
                    self.record_result("World Instantiation", False, 
                                     "World directory not found", time.time() - start_time)
                    return False
            else:
                self.record_result("World Instantiation", False, 
                                 f"Creation failed: {result}", time.time() - start_time)
                return False
    ...
    async def test_taxonomy_guidelines(self) -> bool:
        ...
        try:
            ...
            if "guidelines" not in guidelines_result.lower():
                self.record_result("Taxonomy Guidelines", False, 
                                 "Guidelines generation failed", time.time() - start_time)
                return False
            ...
            if "successfully" in taxonomy_result:
                ...
                if chars_dir.exists():
                    self.record_result("Taxonomy Guidelines", True, 
                                     "Generated guidelines and created taxonomy structure", 
                                     time.time() - start_time)
                    return True
                else:
                    self.record_result("Taxonomy Guidelines", False, 
                                     "Taxonomy directory not created", time.time() - start_time)
                    return False
            else:
                self.record_result("Taxonomy Guidelines", False, 
...
She discovered the magical Orb of Winds during her quest in the Whispering Caverns and now seeks to prevent the Shadow Cult from corrupting the crystal network."""
            ...
            if "successfully" in result:
                ...
                entry_file = self.world_directory / "entries" / "characters" / "lyra-skyward.md"
                if entry_file.exists():
                    ...
                    self.record_result("Entry Creation", True, 
                                     f"Created entry and {len(all_entries) - 1} potential stubs", 
                                     time.time() - start_time)
                    return True
                else:
                    self.record_result("Entry Creation", False, 
                                     "Entry file not created", time.time() - start_time)
                    return False
            else:
                self.record_result("Entry Creation", False, 
                                 f"Entry creation failed: {result}", time.time() - start_time)
                return False
    ...
    async def test_image_generation(self) -> bool:
        ...
        try:
            ...
            if not entry_file.exists():
                self.record_result("Image Generation", False, 
            ...
            if "successfully" in result:
                ...
                image_file = self.world_directory / "images" / "characters" / "lyra-skyward.png"
                if image_file.exists():
                    self.record_result("Image Generation", True, 
                                     "Generated image for character entry", 
                                     time.time() - start_time)
                    return True
                else:
                    self.record_result("Image Generation", False, 
                                     "Image file not created", time.time() - start_time)
                    return False
            else:
                self.record_result("Image Generation", False, 
                                 f"Image generation failed: {result}", time.time() - start_time)
                return False
        ...
        try:
            ...
            if "successfully" in result:
                ...
                if site_dir.exists():
                    ...
                    missing_files = [f for f in required_files if not (site_dir / f).exists()]
                    
                    if missing_files:
                        self.record_result("Site Building", False, 
                    ...
                    self.record_result("Site Building", True, 
                                     f"Built site with {len(html_files)} HTML pages", 
                                     time.time() - start_time)
                    return True
                else:
                    self.record_result("Site Building", False, 
                                     "Site directory not created", time.time() - start_time)
                    return False
            else:
                self.record_result("Site Building", False, 
                                 f"Site building failed: {result}", time.time() - start_time)
                return False
        ...
        try:
            ...
            if index_file.exists():
                ...
                if "gallery" not in content.lower():
                    self.record_result("Content Validation", False, 
            ...
            self.record_result("Content Validation", True, 
    ...
    async def run_all_tests(self) -> Dict:
        ...
        summary = {
            ...
            "world_directory": str(self.world_directory) if self.world_directory else None,
            "results": self.results
        }
        
        self.log(f"Tests completed: {passed}/{total} passed in {total_duration:.2f}s", 
...
async def main():
    ...
    parser.add_argument("--cleanup", action="store_true", 
                       help="Clean up test world after completion")
    parser.add_argument("--verbose", action="store_true", 
                       help="Enable verbose logging")
    parser.add_argument("--base-dir", default=".", 
                       help="Base directory for test world creation")
