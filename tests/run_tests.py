...
class TestSuiteRunner:
    ...
    async def run_unit_tests(self) -> bool:
        ...
        try:
            ...
            from vibe_worldbuilding.tools import entries, site, taxonomy, world
        ...
        try:
            ...
            if duration < 5.0 and result and "successfully" in result[0].text:
                ...
                if self.cleanup:
                    ...
                    perf_dirs = list(
                        (self.test_dir / "performance").glob("perf-test-*")
                    )
                ...
                self.log(
                    f"❌ Performance test failed (too slow: {duration:.2f}s)", "ERROR"
                )
...
async def main():
    ...
    # Run the test suite
    runner = TestSuiteRunner(verbose=args.verbose, cleanup=not args.no_cleanup)
